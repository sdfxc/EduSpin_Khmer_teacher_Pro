...
export function GlassLiquidOverlay({
  ...
  const getVariantStyles = () => {
    ...
    if (variant === 'orange-glass' || variant === 'amber-glass') {
      return 'bg-gradient-to-b from-orange-400 via-orange-500 to-amber-700 border-t border-white/85 border-b border-orange-950/80 shadow-[0_10px_35px_rgba(249,115,22,0.7),0_0_28px_rgba(251,146,60,0.55),inset_0_2.5px_4.5px_rgba(255,255,255,0.9),inset_0_-2.5px_4.5px_rgba(0,0,0,0.4)]';
    }
  ...
  const content = (
    <div
      className={`absolute inset-0 rounded-full border backdrop-blur-2xl overflow-hidden pointer-events-none transition-colors ${getVariantStyles()} ${className}`}
    >
      {/* Top Specular Glare Dome Reflection (ចំណាំងពន្លឺកោងមូលតំណក់ទឹកថ្លា) */}
      <div
        className={`absolute top-0 inset-x-1.5 h-[54%] rounded-t-full pointer-events-none ${
          isColorVariant
